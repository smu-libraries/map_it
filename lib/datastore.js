...
/**
 * Initializes the datastore. This function is synchronous.
 ...
 * @returns {object[]} The list of items that match the given criteria, or [] if no matching item is found.
 */
 ...
 */
let get_libraries = () => {
  return _search({ doctype: 'library' });
}
...
let get_library = (library_code) => {
  return _search({ doctype: 'library', code: library_code });
}
...
let get_locations = (library_code) => {
  return _search({ doctype: 'location', parent: library_code });
}
...
let get_location = (library_code, location_code) => {
  return _search({ doctype: 'location', parent: library_code, code: location_code });
}
...
let get_ranges = (library_code, location_code) => {
  return _search({ doctype: 'range', parent: `${library_code} ${location_code}` });
}
...
let get_range = (library_code, location_code, range_code) => {
  return _search({ doctype: 'range', parent: `${library_code} ${location_code}`, code: range_code });
}
