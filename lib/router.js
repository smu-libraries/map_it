 ...
 * @promises {object[]} The list of documents with hyperlinks inserted.
 */
let insert_links = (docs, library_code, location_code) => {
    ...
    switch (doc.doctype) {
      case 'library':
        doc.links = [
          {
            rel: 'self',
            href: `/${APP}/${VERSION}/libraries/${doc.code}`
          },
          {
            rel: 'locations',
            href: `/${APP}/${VERSION}/libraries/${doc.code}/locations`
          }
        ];
        break;
      ...
      case 'location':
        doc.links = [
          {
            rel: 'self',
            href: `/${APP}/${VERSION}/libraries/${library_code}/locations/${doc.code}`
          },
          {
            rel: 'library',
            href: `/${APP}/${VERSION}/libraries/${library_code}`
          },
          {
            rel: 'ranges',
            href: `/${APP}/${VERSION}/libraries/${library_code}/locations/${doc.code}/ranges`
          }
        ];
        break;
      ...
      case 'range':
        doc.links = [
          {
            rel: 'self',
            href: `/${APP}/${VERSION}/libraries/${library_code}/locations/${location_code}/ranges/${doc.code}`
          },
          {
            rel: 'library',
            href: `/${APP}/${VERSION}/libraries/${library_code}`
          ...
          {
            rel: 'location',
            href: `/${APP}/${VERSION}/libraries/${library_code}/locations/${location_code}`
          },
          {
            ref: 'map',
            href: `/${APP}/${VERSION}/libraries/${library_code}/locations/${location_code}/ranges/${doc.code}?view=map`
          }
        ];
        break;
...
router.get_libraries = (req, res) => {
  return datastore.get_libraries()
    .then((docs) => { return insert_links(docs); })
...
router.get_library = (req, res) => {
  ...
  return datastore.get_library(params.library_code)
    .then((docs) => { return insert_links(docs, params.library_code); })
    .then((docs) => { res.json(docs); });
};
...
router.get_location = (req, res) => {
  ...
  return datastore.get_location(params.library_code, params.location_code)
    .then((docs) => { return insert_links(docs, params.library_code, params.location_code); })
    .then((docs) => { res.json(docs); });
};
